...
export const SmesherHistoryItemSchema = z.discriminatedUnion('state', [
  ...
  SmesherHistoryItem(SmesherState.WAIT_FOR_ATX_SYNCED, 'waitForAtxSynced', z.object({})),
  SmesherHistoryItem(SmesherState.RETRYING, 'retrying', z.object({
    message: z.string(),
  })),
  SmesherHistoryItem(SmesherState.WAITING_FOR_POET_REGISTRATION_WINDOW, 'waitingForPoetRegistrationWindow', z.object({})),
  SmesherHistoryItem(SmesherState.POET_CHALLENGE_READY, 'poetChallengeReady', z.object({})),
  SmesherHistoryItem(SmesherState.POET_REGISTERED, 'poetRegistered', z.object({
    registrations: z.array(PoETRegistrationSchema),
  })),
  SmesherHistoryItem(SmesherState.WAIT_FOR_POET_ROUND_END, 'waitForPoetRoundEnd', z.object({
    roundEnd: z.string().datetime(),
    publishEpochEnd: z.string().datetime(),
  })),
  SmesherHistoryItem(SmesherState.POET_PROOF_RECEIVED, 'poetProofReceived', z.object({
    poetUrl: z.string().url(),
  })),
  SmesherHistoryItem(SmesherState.GENERATING_POST_PROOF, 'generatingPostProof', z.object({})),
  SmesherHistoryItem(SmesherState.POST_PROOF_READY, 'postProofReady', z.object({})),
  SmesherHistoryItem(SmesherState.ATX_READY, 'atxReady', z.object({})),
  SmesherHistoryItem(SmesherState.ATX_BROADCASTED, 'atxBroadcasted', z.object({
    atxId: Base64Schema,
  })),
...
export const SmesherIdentitiesSchema = z.record(z.object({
